...
- VnExpress returns range totals, banner/order rows, and (when available) daily `detail` rows. The collector groups daily impressions and clicks by date, derives CTR from those sums, and reconciles daily totals against the period total. Dates without returned detail remain absent rather than being filled with zero. Older responses without daily details remain usable as complete period snapshots and carry a warning; mismatched daily totals keep the previous snapshot. Live provider access was verified with an authorized request for 2026-09-01–2026-09-30.
