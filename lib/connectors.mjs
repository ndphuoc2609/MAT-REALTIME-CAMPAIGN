...
export class SourceError extends Error {constructor(message,status='error'){super(message);this.status=status;}}
...
async function probe24hReport(page, link, { allowInsecureHttp = false } = {}) {
  ...
  if (result.failure) {
    ...
    if (failure) throw new SourceError(failure.message, failure.status);
  }
...
async function login24h(page, link, { directory, job, update } = {}) {
  ...
  if (circuit?.open) {
    throw new SourceError('Tự đăng nhập 24h đang tạm dừng sau lỗi xác thực trước đó. Sửa credentials hoặc OTP/CAPTCHA rồi dùng chức năng đặt lại mạch xác thực của quản trị viên.', 'auth_blocked');
  }
  job.status = 'running';
...
export async function ensureAuthenticatedSession(page, link, { directory, job = {}, update = async () => {}, force = false, login = false, probe = true, loginHandler = login24h, beforeLogin = async currentPage => currentPage } = {}) {
  ...
  try {
    ...
    if (link.connector === '24h') await probe24hReport(currentPage, link, { allowInsecureHttp });
...
async function collectUnlocked(link,{directory,projectRoot,job = {},update = async () => {},login=false,lockHeld=false,profileLock=null,browser=chromium,adapterMap=adapters,ensureSession=ensureAuthenticatedSession,loginHandler=login24h,retryBudget=2}) {
  ...
  try {
    ...
    const readWithRecovery = async (from, to) => {
      try { return await adapter.read(page,link,from,to,{ secureTransport: autoLoginEnabled && !allowInsecureHttp }); }
        ...
        if (error?.status !== 'auth_required' || authRecoveryUsed || !autoLoginEnabled || !['24h', 'admicro-pc', 'admicro-mobile'].includes(link.connector) || Number(job.retries || 0) >= Number(retryBudget)) throw error;
