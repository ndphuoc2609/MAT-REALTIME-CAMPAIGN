...
import { collectVnExpressReport, VNEXPRESS_CONNECTOR } from './vnexpress-report.mjs';
...
function canResumeInvalidCredentials(circuit, job) {
  return circuit?.reason === 'invalid_credentials'
    && validScheduledDate(job?.scheduledDate)
    && circuit.lastScheduledDate !== job.scheduledDate;
}
function alreadySubmittedForSchedule(circuit, job) {
  if (typeof circuit?.lastScheduledDate !== 'string') return false;
  if (job?.scheduledDate == null || job.scheduledDate === '') return false;
  return !validScheduledDate(job.scheduledDate) || circuit.lastScheduledDate === job.scheduledDate;
}
...
function blocked24hCircuitError(circuit, temporary, job = {}) {
  const scheduledAttempt = validScheduledDate(job?.scheduledDate) && circuit?.lastScheduledDate === job.scheduledDate;
  if (temporary?.fresh) {
    const minutes = Math.max(1, Math.ceil(temporary.remainingMs / 60_000));
    const attempt = scheduledAttempt ? `Lịch ngày ${job.scheduledDate} đã dùng lượt submit (${circuit.reason}); ` : '';
    return new SourceError(`${attempt}tự đăng nhập 24h đang tạm dừng sau lỗi xác thực; thử lại sau khoảng ${minutes} phút hoặc dùng phiên thủ công.`, 'auth_blocked');
  }
  if (scheduledAttempt && TEMPORARY_24H_AUTH_REASONS.has(circuit?.reason)) {
    return new SourceError(`Lịch ngày ${job.scheduledDate} đã dùng lượt submit (${circuit.reason}); lịch ngày tiếp theo có thể thử lại, hoặc chạy thủ công hay nhờ quản trị viên reset mạch xác thực.`, 'auth_blocked');
  }
  if (circuit?.reason === 'invalid_credentials') {
    if (validScheduledDate(job?.scheduledDate) && circuit.lastScheduledDate === job.scheduledDate) {
      return new SourceError(`Tự đăng nhập 24h đã dùng một lần thử trong lịch ngày ${job.scheduledDate} sau khi credentials bị từ chối. Lịch tự động ngày tiếp theo có thể thử lại; sửa credentials trên máy chủ hoặc quản trị viên có thể reset mạch xác thực.`, 'auth_blocked');
    }
...
const validIsoDate=value=>typeof value==='string'&&/^\d{4}-\d{2}-\d{2}$/.test(value)&&Number.isFinite(Date.parse(`${value}T00:00:00.000Z`))&&new Date(`${value}T00:00:00.000Z`).toISOString().slice(0,10)===value;
...
function parse24Payload(payload){const rows=findRows(payload);if(!rows.length)throw new SourceError('Response 24h không có danh sách dữ liệu.','schema_error');const normalized=rows.map((row,i)=>{const sourceDate=exact(row,['c_date','date','ngày','ngay','day']);const date=normalize24Date(sourceDate);if(sourceDate!=null&&String(sourceDate).trim()!==''&&!validIsoDate(date))throw new SourceError('24h trả về ngày báo cáo sai định dạng.','schema_error');const id=exact(row,['pk_dfp_lineitem','lineitem_id','lineitem','line_item','id'])??String(i+1);const creative=exact(row,['c_name','name','campaign','creative','lineitem','line_item'])??null;const position=exact(row,['c_website','position','site','placement','ad_unit','vị trí','vi tri'])??null;return {raw:row,metric:metric24(row,{daily:Boolean(date)}),date,id,creative,position};});if(!normalized.some(row=>row.date)&&rows.some(row=>['c_sum_impressions','sum_impressions','c_sum_clicks','sum_clicks','c_sum_spend'].some(key=>Object.hasOwn(row,key))))throw new SourceError('24h trả về dòng dữ liệu theo ngày nhưng thiếu ngày.','schema_error');return merge24(normalized);}
...
async function save24hReportFilter(page, endpointPath, from, to, { allowInsecureHttp = true, secureTransport = false, accessDeniedAsAuth = false } = {}) {
  if (!validIsoDate(from) || !validIsoDate(to) || from > to) throw new SourceError('Khoảng ngày báo cáo 24h không hợp lệ.', 'invalid_date');
  const screenName = endpointPath.replace(/^\/ocm\/ajax\//, '/ocm/');
  const result = await page.evaluate(async ({ screenName, from, to, allowInsecureHttp, secureTransport }) => {
    const endpoint = new URL('/ocm/sync_dfp/act_save_user_custom_report/', location.origin);
    if (endpoint.hostname !== 'khachhang.24h.com.vn' || endpoint.port) return { failure: { status: 0, transportSecurity: true } };
    for (const [field_name, value] of [
      ['c_statistic_from_date', from.split('-').reverse().join('-')],
      ['c_statistic_to_date', to.split('-').reverse().join('-')],
      ['c_statistic_date_range_type', 'CUSTOM_DATE']
    ]) {
      const response = await fetch(endpoint.href, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', 'X-Requested-With': 'XMLHttpRequest' },
        body: new URLSearchParams({ screen_name: screenName, field_name, field_value: value }),
        credentials: 'include'
      });
      ...
      const transportSecurity = finalUrl.hostname !== 'khachhang.24h.com.vn'
        || Boolean(finalUrl.port)
        || (!allowInsecureHttp && finalUrl.protocol !== 'https:')
        || (secureTransport && finalUrl.protocol !== 'https:');
      if (!response.ok || redirectedToLogin || passwordForm || html || accessDenied || transportSecurity) {
        return { failure: { status: response.status, contentType, redirectedToLogin, passwordForm, html, accessDenied, transportSecurity } };
      }
    }
    return { saved: true };
  }, { screenName, from, to, allowInsecureHttp, secureTransport });
  if (result.failure) {
    const failure = classify24Response(result.failure);
    if (failure) {
      if (failure.status === 'access_denied' && accessDeniedAsAuth) throw new SourceError('Phiên đăng nhập 24h không còn hợp lệ. Cập nhật phiên nguồn rồi chạy lại.', 'auth_required');
  ...
  if (result.saved !== true) throw new SourceError('Không xác minh được bộ lọc kỳ báo cáo 24h.', 'schema_error');
}
...
async function probe24hReport(page, link, { allowInsecureHttp = false, accessDeniedAsAuth = false } = {}) {
  ...
  await save24hReportFilter(page, endpointPath, link.from, link.to, { allowInsecureHttp, accessDeniedAsAuth });
...
async function read24h(page, link, from, to, { secureTransport = false } = {}) {
  ...
  const endpointPath = endpoint24hPath(link.url);
  await save24hReportFilter(page, endpointPath, from, to, { secureTransport });
...
function checkedLoginRedirect(value, expected, { allowInsecureHttp = false, allowPathChange = false } = {}) {
  ...
  if (!protocolOk || target.hostname !== 'khachhang.24h.com.vn' || target.port !== expected.port || (!allowPathChange && target.pathname !== expected.pathname)) {
    throw new SourceError('Endpoint đăng nhập 24h chuyển hướng khỏi transport được phép hoặc action đã xác minh.', 'transport_security');
  }
  return target;
}
...
async function installLoginTransportGuard(page, action, { allowInsecureHttp = false } = {}) {
  ...
  const submit = async route => {
    try {
      ...
      checkedLoginRedirect(response.headers()['location'], actionUrl, { allowInsecureHttp, allowPathChange: true });
      const responseUrl = new URL(response.url());
      const responseProtocolOk = responseUrl.protocol === 'https:' || (allowInsecureHttp && responseUrl.protocol === 'http:');
...
async function login24h(page, link, { directory, job, update } = {}) {
  ...
  if (circuit?.open) {
    ...
    if (alreadySubmittedForSchedule(circuit, job)) throw blocked24hCircuitError(circuit, temporary, job);
    if (circuit.reason === 'invalid_credentials' && !canResumeInvalidCredentials(circuit, job)) {
      throw blocked24hCircuitError(circuit, temporary, job);
    }
    if (!temporary?.valid || temporary.fresh) {
      if (!canResumeInvalidCredentials(circuit, job)) throw blocked24hCircuitError(circuit, temporary, job);
    }
  }
  job.status = 'running';
  ...
  await tripAuthCircuit(directory, '24h', 'login_in_progress', { scheduledDate: validScheduledDate(job?.scheduledDate) ? job.scheduledDate : undefined });
  ...
  if (loginResult === 'invalid_credentials') {
    await tripAuthCircuit(directory, '24h', 'invalid_credentials', { scheduledDate: validScheduledDate(job?.scheduledDate) ? job.scheduledDate : undefined });
  ...
  try {
    ...
    const loginAndVerify = async () => {
      ...
      if (link.connector === '24h') {
        ...
        const resumeInvalidCredentials = canResumeInvalidCredentials(circuit, job);
        if (circuit?.open && temporary && !temporary.valid) throw blocked24hCircuitError(circuit, temporary, job);
        if (circuit?.open && temporary && replacementPage) {
          try {
            ...
            if (temporary.fresh) throw blocked24hCircuitError(circuit, temporary, job);
          }
        }
        // A pending candidate may already contain a verified session. Let the
        // bounded read-only proof above run before blocking another same-day
        // credential submission.
        if (circuit?.open && alreadySubmittedForSchedule(circuit, job)) throw blocked24hCircuitError(circuit, temporary, job);
        if (circuit?.open && temporary?.fresh) throw blocked24hCircuitError(circuit, temporary, job);
        if (circuit?.open && !temporary && !resumeInvalidCredentials) throw blocked24hCircuitError(circuit, temporary, job);
      }
...
async function collectUnlocked(link,{directory,projectRoot,job = {},update = async () => {},login=false,lockHeld=false,profileLock=null,browser=chromium,adapterMap=adapters,ensureSession=ensureAuthenticatedSession,loginHandler=login24h,retryBudget=2, ...googleOptions}) {
  ...
  if (link.connector === VNEXPRESS_CONNECTOR) return collectVnExpressReport(link, { ...googleOptions, job, update });
  ...
  try {
    ...
    if(period.daily?.length){if(link.connector==='24h'&&period.daily.some(row=>!validIsoDate(row.date)||row.date<link.from||row.date>link.to))throw new SourceError('24h trả về ngày báo cáo sai định dạng hoặc nằm ngoài khoảng ngày trên link.','schema_error');daily.push(...period.daily);dailyDetails.push(...(period.dailyDetails||period.details).map(row=>({...row,date:row.date||null})));job.message=`Đã nhận ${daily.length} dòng ngày từ nguồn`;await update();}
...
export async function collect(link, options = {}) {
  if (link.connector === 'google-ads' || link.connector === META_CONNECTOR || link.connector === VNEXPRESS_CONNECTOR) return collectUnlocked(link, options);
