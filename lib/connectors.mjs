  ...
  try {
    ...
    if (link.connector === '24h') {
      try {
        ...
        // not established the refreshed session yet. Allow one bounded
        // re-login, then preserve access_denied if the protected probe still
        // rejects the session. The second probe is intentionally outside this
        // recovery catch so this cannot become an auth retry loop.
