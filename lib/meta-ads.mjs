...
function apiError(payload, status) {
  ...
  if (code === 10 || code === 200 || code === 4701 || status === 403) return new SourceError('Token Meta Ads không có quyền đọc tài khoản hoặc báo cáo này.', 'access_denied');
...
export async function validateMetaSavedReport(link, { env = process.env } = {}) {
...
export async function collectMetaAds(link, {
  ...
  const filters = link.metaReport?.filters || (link.metaReport?.savedReportId ? (() => {
    const reportMonth = /^\d{4}-(0[1-9]|1[0-2])$/.test(link.reportMonth || '') ? link.reportMonth : link.from.slice(0, 7);
    const [year, month] = reportMonth.split('-');
    return { campaignName: `KMBH T${Number(month)}/${year}`, objective: null, hadDelivery: '1' };
  })() : null);
  const invalidCampaign = typeof filters?.campaignName !== 'string' || !filters.campaignName.trim();
