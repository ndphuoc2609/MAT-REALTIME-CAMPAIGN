...
export const GOOGLE_ADS_QUERY = `SELECT segments.date, campaign.advertising_channel_type, metrics.impressions, metrics.clicks FROM campaign${DATE_FILTER}`;
...
function metricResult(impressions, clicks) {
...
export function aggregateGoogleAdsRows(rows, { from, to } = {}) {
  ...
  const aggregate = aggregateMetricRows(filtered);
  const unknownTypes = [...new Set(filtered.map(row => row.channelType).filter(type => !channelKey(type)))].sort(compareText);
  const channels = {
    gdn: aggregateMetricRows(filtered.filter(row => channelKey(row.channelType) === 'gdn')),
    sem: aggregateMetricRows(filtered.filter(row => channelKey(row.channelType) === 'sem')),
    unknown: Object.fromEntries(unknownTypes.map(type => [type, aggregateMetricRows(filtered.filter(row => row.channelType === type))]))
  };
  return { ...aggregate, details: [], channels };
}
...
export async function collectGoogleAds(link, {
  ...
  return {
    ...
    viewerDefinition: 'Google Ads metrics tổng theo ngày, phân tách GDN (DISPLAY) và SEM (SEARCH), cùng breakdown theo từ khóa, giới tính, độ tuổi; không hiển thị chi tiết campaign.'
  };
}
