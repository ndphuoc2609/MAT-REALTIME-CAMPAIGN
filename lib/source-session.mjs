...
export async function tripAuthCircuit(directory, connector, reason = 'authentication_failed') {
  const state = await readCircuit(directory);
  state[profileName(connector)] = { open: true, reason, at: new Date().toISOString() };
