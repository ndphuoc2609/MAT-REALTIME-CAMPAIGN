...
function identifyMetaReport(url, input = {}) {
  ...
  // Saved report IDs stay local; the campaign filter is reconstructed from
  // the selected reporting month because Ads Manager omits it from the URL.
  const hasInlineDefinition = ['breakdowns', 'metrics', 'filter_set', 'filtering', 'filters']
  ...
  if (flat.selected_report_id && !hasInlineDefinition) {
    ...
    let from = input.from || flat.since || flat.from || dateRange?.since || dateRange?.start_date || range?.[1];
    let to = input.to || flat.until || flat.to || dateRange?.until || dateRange?.end_date || range?.[2];
    ...
    const filterMonth = month(input.reportMonth) || from?.slice(0, 7);
    const [year, monthNumber] = filterMonth?.split('-') || [];
    return {
      account, savedReportId,
      filters: filterMonth ? { campaignName: `KMBH T${Number(monthNumber)}/${year}`, campaignOperator: 'CONTAIN', objective: null, hadDelivery: '1' } : null,
      metrics: ['clicks', 'impressions', 'ctr'], from, to
    };
  }
