...
function normalizePayload(payload, link) {
  ...
  return {
    ...
    reconciliation: { dailyTotal: { impressions: null, clicks: null, ctr: null }, differences: { impressions: null, clicks: null }, status: 'not_available' },
    complete: true, fetchedAt: new Date().toISOString(), from: link.from, to: link.to, timezone: null,
    method: 'VnExpress report API · một yêu cầu theo khoảng ngày', verification: 'live',
    warnings: ['API VnExpress chỉ trả về dữ liệu theo khoảng ngày; không có phân rã theo ngày.'],
...
export async function collectVnExpressReport(link, {
  ...
  console.info('[VnExpress] API response', { httpStatus: response.status, payload });
  let result;
