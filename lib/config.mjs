...
// credentials are supported for server-side source integrations, but are never
// logged or returned to a caller.
const allowed = new Set(['PORT', 'HOST', 'DATA_DIR', 'DATABASE_URL', 'SYNC_INTERVAL_MINUTES', 'TRUST_PROXY', 'COOKIE_SECURE', 'PUBLIC_ORIGIN', 'CHROME_BIN', 'HOUSEKEEPING_ENABLED', 'HOUSEKEEPING_INTERVAL_MINUTES', 'HOUSEKEEPING_SNAPSHOT_RETAIN', 'HOUSEKEEPING_JOB_RETENTION_DAYS', 'HOUSEKEEPING_CACHE_ENABLED', 'SOURCE_24H_USERNAME', 'SOURCE_24H_PASSWORD', 'SOURCE_24H_AUTO_LOGIN', 'SOURCE_24H_ALLOW_INSECURE_HTTP', 'SOURCE_ADMICRO_USERNAME', 'SOURCE_ADMICRO_PASSWORD', 'SOURCE_ADMICRO_AUTO_LOGIN', 'SOURCE_VNEXPRESS_USER_NAME', 'SOURCE_VNEXPRESS_API_SECRET_KEY', 'GOOGLE_ADS_CLIENT_ID', 'GOOGLE_ADS_CLIENT_SECRET', 'GOOGLE_ADS_REFRESH_TOKEN', 'GOOGLE_ADS_DEVELOPER_TOKEN', 'GOOGLE_ADS_CUSTOMER_ID', 'GOOGLE_ADS_LOGIN_CUSTOMER_ID', 'META_ACCESS_TOKEN', 'META_AD_ACCOUNT_ID', 'META_GRAPH_VERSION']);

export function metaAdsConfig(env = process.env) {
  const accessToken = String(env.META_ACCESS_TOKEN || '').trim();
  const accountId = String(env.META_AD_ACCOUNT_ID || '').trim().replace(/^act_/, '');
  const graphVersion = String(env.META_GRAPH_VERSION || 'v26.0').trim();
  if (!accessToken) throw new Error('Thiếu cấu hình META_ACCESS_TOKEN cho Meta Ads.');
  if (!/^\d{5,20}$/.test(accountId)) throw new Error('Cấu hình META_AD_ACCOUNT_ID không hợp lệ.');
  if (!/^v\d+\.\d+$/.test(graphVersion)) throw new Error('Cấu hình META_GRAPH_VERSION không hợp lệ.');
  return { accessToken, accountId, graphVersion };
}
